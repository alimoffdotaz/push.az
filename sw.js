...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      if (action === 'snooze') {
        const ackOk = isRealReminder ? await callAck(reminderId, 'snooze', 10) : true;
        if (ackOk) {
          await notifyClients({ type: 'reminder-snoozed', reminderId, ackOk: true });
        } else {
          await notifyClients({ type: 'reminder-snooze-failed', reminderId });
          await focusClient();
        }
        return;
      }
...
async function callAck(reminderId, action, minutes = 10) {
  try {
    ...
    if (!res.ok) {
      console.warn('[sw] ack failed:', res.status);
      return false;
    }
    return true;
  } catch (err) {
