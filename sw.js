...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      if (action === 'snooze') {
        const minutes = 10;
        const serverAcked = isRealReminder ? await callAck(reminderId, 'snooze', minutes) : false;
        await notifyClients({ type: 'reminder-snoozed', reminderId, minutes, serverAcked });
        return;
      }
