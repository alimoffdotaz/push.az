...
async function processOneReminder(env, r, vapid, now) {
  ...
  // Telegram ne zavisit ot Web Push/VAPID: linked-polzovatel ne dolzhen teryat' reminder.
  if (env.TELEGRAM_BOT_TOKEN) {
    try {
      const tgResult = await tgSendReminderToUser(env, r.user_id, reminder, built.text, attempt, MAX_ATTEMPTS, newsLine);
      if (Number(tgResult?.sent || 0) > 0) anyOk = true;
    } catch (err) {
  ...
  if (!anyOk) {
    // Net rabochego kanala (net TG, net VAPID/device'ev ili vse device'y gone) — ne krutim cron kazhduyu minutu.
    await env.DB.prepare(
