...
async function handleUpsertReminder(request, env, user) {
  ...
  // Yesli updateim — proverim vladel'tsa i zashchitim delivery-state ot stale sync.
  ...
  const now = Date.now();
  const clientUpdatedAt = Number(updatedAt || 0);
  if (existing && clientUpdatedAt > 0 && Number(existing.updated_at || 0) > clientUpdatedAt) {
  ...
  if (!existing) {
    ...
    const allowScheduleWrite = clientUpdatedAt > 0;
    const nextFireAt = allowScheduleWrite ? fireAt : existing.fire_at;
    const nextRepeat = allowScheduleWrite ? repeat : (existing.repeat || 'none');
    const scheduleChanged =
      allowScheduleWrite &&
      (Number(existing.fire_at) !== Number(fireAt) || (existing.repeat || 'none') !== repeat);
    if (scheduleChanged) {
      await env.DB.prepare(
        ...
        .bind(user.userId, title, note, nextFireAt, nextRepeat, tone, now, id)
        .run();
    } else {
      await env.DB.prepare(
        `UPDATE reminders SET
...
async function runScheduler(env) {
  ...
  if (!vapid) {
    console.warn('[scheduler] VAPID not configured, continuing with non-Web-Push channels');
  }
...
async function processOneReminder(env, r, vapid, now) {
  ...
  if (vapid) for (const d of devices) {
  ...
  if (!anyOk) {
    // Net rabochikh kanalov: ne spinim kazhduyu minutu, probuyem pozzhe.
    await env.DB.prepare(
