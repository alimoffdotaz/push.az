  ...
  await env.DB.prepare(
    `INSERT INTO reminders
     ...
     ON CONFLICT(id) DO UPDATE SET
       ...
       tone = excluded.tone,
       status = CASE
         WHEN excluded.fire_at != reminders.fire_at OR excluded.repeat != reminders.repeat
           THEN 'active'
...
async function processOneReminder(env, r, vapid, now) {
  ...
  // Snachala shlyom v Telegram: eto samostoyatelnyy kanal dostavki, ne fallback ot Web Push.
  let tgSent = 0;
  if (env.TELEGRAM_BOT_TOKEN) {
    try {
      const tgResult = await tgSendReminderToUser(env, r.user_id, reminder, built.text, attempt, MAX_ATTEMPTS, newsLine);
      tgSent = Number(tgResult?.sent || 0);
    } catch (err) {
  ...
  if (!anyOk) {
    // Net dostupnykh kanalov (net device'ev, VAPID vyklyuchen ili vse device'y gone).
    await env.DB.prepare(
