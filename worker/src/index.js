...
async function processOneReminder(env, r, vapid, now) {
  ...
  if (env.TELEGRAM_BOT_TOKEN) {
    try {
      ...
      telegramFailed = Number(tgResult?.failed || 0) > 0;
    } catch (err) {
      telegramFailed = true;
  ...
  // Nakhodim vse aktivnye device'y polzovatelya. Telegram delivery above must
  // not depend on Web Push subscriptions being present or healthy.
  ...
  if (!devices.length && !telegramSent) {
    // Net dostavki — prosto prodvigayem next_attempt, chtoby ne lomat' schedule.
    await env.DB.prepare(
  ...
  if (!anyOk) {
    // Dostavit' nekuda: vse device'y gone ili ne ostalos' zhivykh Telegram-chatov.
    await env.DB.prepare(
      `UPDATE reminders SET next_attempt_at = ?1, updated_at = ?2 WHERE id = ?3`,
    )
