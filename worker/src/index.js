...
async function processOneReminder(env, r, vapid, now) {
  ...
  let tgSent = 0;
  // Telegram delivery counts as a real attempt even when Web Push is unavailable.
  if (env.TELEGRAM_BOT_TOKEN) {
    try {
  ...
  const anyDelivered = anyOk || tgSent > 0;

  if (!anyDelivered && anyNonGoneError) {
    // Vse popytki v etu iteratsiyu upali — ne prodvigayem schetchik
    return;
  }
  if (!anyDelivered) {
    // Net zhivykh kanalov dostavki — otlozhim proverku, chtoby ne krutit' cron kazhduyu minutu.
    await env.DB.prepare(
