    ...
    DB: {
      prepare(sql) {
        const statement = {
          ...
          async all() {
            if (sql.includes('FROM reminders') && sql.includes('next_attempt_at <=')) {
              return { results: due };
            }
...
test('scheduler advances after Telegram delivery even when VAPID is not configured', async (t) => {
...
test('scheduler backs off when no Web Push devices or Telegram chats can receive', async (t) => {
  t.mock.method(Date, 'now', () => NOW);
  const fetchMock = t.mock.method(globalThis, 'fetch', async () => {
    throw new Error('fetch should not be called without Telegram chats');
  });

  const { env, updates } = createEnv({ chats: [] });
